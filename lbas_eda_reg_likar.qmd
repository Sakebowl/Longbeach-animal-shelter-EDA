...
binned_residuals(lb_simple_merged_mdl, term = "animal_type")
...
pred_probs <- predict(lb_simple_merged_mdl, type = "response")

# Calculate ROC curve
roc_curve <- roc(lb_analysis$adopted, pred_probs)
plot(roc_curve)  # Visualize
auc(roc_curve)   # Get AUC (0.5 = random, 1 = perfect)
```
...
```{r}
# Get predicted probabilities
pred_probs <- predict(lb_complex_mdl, type = "response")
...
lb_analysis_copy <- lb_analysis
lb_analysis_copy$pred_prob <- predict(lb_complex_mdl, type = "response")
ggplot(lb_analysis_copy, aes(x = pred_prob, y = adopted)) + 
